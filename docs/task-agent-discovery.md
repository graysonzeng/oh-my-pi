...
- `output-truncation: false` (normalized to `outputTruncation`) skips model-family tool-output byte/line clamps for that spawn — `runSubprocess` applies it as a `modelOptimization.outputTruncation.enabled: false` override (`src/task/executor.ts`). The family profile stays unchanged; ordinary Grok sessions keep their clamps. Defaults to enabled when the field is absent.
