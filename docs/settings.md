...
| `providers.openai-codex.codeMode`           | enum    | `off`             | Codex fallback for programmatic tool calling when `tools.ptc.mode` is `off`. For `code_mode_only` models (GPT-5.6 Sol/Terra/Luna), the direct tool surface collapses to `eval`/`ask`/`todo`/`yield`/`think`/`checkpoint`/`rewind`/`new_context` and every other session tool is invoked from `eval` cells via its `tool.<name>()` bridge. `auto` follows the model catalog's `tool_mode` flag; `on` forces it for any Codex model; `off` (default) leaves the full direct surface unless generic PTC is on. The turn metadata carries codex-rs's `tool_namespaces_info` exposure snapshot while a Codex session is active. |
| `providers.openai-codex.codeModeDirectTools` | array   | `[]`              | Extra tool names to keep directly callable alongside the Code Mode keep-set when Codex Code Mode is active; entries that are not enabled in the session are ignored. |
...
| `tools.ptc.mode`               | enum    | `off`   | Programmatic tool calling for any provider. `on` collapses the direct surface to `eval`/`ask`/`todo`/`yield`/`think`/`checkpoint`/`rewind`/`new_context` whenever JS eval is available; other enabled tools run from eval cells via `tool.<name>()`, with `catalog.searchTools`/`describeTools` recovering omitted schemas. `auto` follows a model's `code_mode_only` catalog flag. `off` (default) leaves the full direct surface; Codex can still enable Code Mode via `providers.openai-codex.codeMode`. `--ptc` and RPC `set_ptc_mode` override per run. |
| `tools.ptc.directTools`        | array   | `[]`    | Extra tool names to keep directly callable alongside the PTC keep-set; entries that are not enabled in the session are ignored. |
