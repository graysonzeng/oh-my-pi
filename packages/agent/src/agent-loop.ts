...
import { SPECULATIVE_STREAM_SESSION } from "./types";
...
async function executeToolCalls(
	...
	telemetry: AgentTelemetry | undefined,
	invokeAgentSpan: Span | undefined,
): Promise<{ toolResults: ToolResultMessage[]; additionalContext?: string; terminalAdmission: boolean }> {
		...
		await runInActiveSpan(toolSpan, async () => {
			try {
				...
				if (speculativeOutcome) {
					...
					const preserveOmitted = record.toolCall.name === "read_omitted_content";
					const coerced = coerceToolResult(speculativeOutcome.result, preserveOmitted);
						...
						rawResult = await tool.execute(
							...
							partialResult => {
								stream.push({
									...
									toolName: toolCall.name,
									args: executionArgs,
									partialResult: coerceToolResult(
										partialResult,
 ...
 */
function prestartSourceFor(source: AsideInterruptSource | undefined): SyntheticResultSource {
