...
- Added quality-gated workflow optimization Phase 0–2 contracts: typed exact-hash context dedupe with verified `artifact://` recovery and fail-safe inline fallback, versioned ContextLedger provider/estimate provenance, an executable 30-case live-fixture suite, explicit live provider/model run fingerprints, and single-lever compiler activation that remains shadowed without evidence and rollout approval.
...
### Fixed
- Fixed logical-session isolation by clearing checkpoint/rewind runtime state during `AgentSession.newSession()`, so completed rewind evidence cannot leak into the next session.
- Fixed Bash repeated-failure identity to hash complete Git HEAD/index/worktree/non-ignored-untracked state plus invocation environment/timeout/PTY state; state changes invalidate prior attempts, while missing or racing Git authority fails open without identical-failure advice.
- Fixed live fixed-model workflow benchmarks to use strict catalog-family profile identity with intentional legacy degraded routing: provenance now accepts that route only when every required child stage has exact attested provider/model evidence, selected-profile agreement, and zero fallback/skip ambiguity.
- Fixed per-turn history pruning to roll back in-memory branch and live-agent tool-result mutations when persistence fails, while preserving the single-rewrite success path.
...
- Live benchmark child provenance now derives exclusively from the child workflow's persisted runtime evidence and routing audit (`verifyLiveWorkflowProvenance`): every required model-backed stage must attest the exact fixed provider/model/checkpoint/effort, and missing, mixed, fallback, or skipped evidence yields unknown provenance with live acceptance failing closed; the outer session model is no longer treated as child identity.
- Workflow context persistence/verification failures stay fail-open-inline: invalid or unverifiable stored context artifacts are removed and never used as recovery evidence, the runner is called exactly once, and no orphan artifact or receipt survives.
- Policy compiler levers remain shadow-only: a self-consistent SHA-256 experiment receipt cannot authorize `active` without a canonical benchmark-report/current-candidate/model-facts/approval authority (`verified_rollout_authority_unavailable`), and production consumers strip raw active gates.
