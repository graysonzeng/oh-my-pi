...
import { cfgWorkflowPresentationOptimizationEnabled } from "../../src/config/workflow-settings";
...
describe("P2 presentation policy defaults + settings gate", () => {
	...
	it("settings schema default for workflow.presentationOptimization.enabled is false", () => {
		expect(cfgWorkflowPresentationOptimizationEnabled.default).toBe(false);
	});
...
describe("P2 xd:// expand + restricted child", () => {
	it("parseXdUrl keeps the device form and rejects presentation paths", () => {
		expect(parseXdUrl("xd://bash")).toEqual({ name: "bash" });
		expect(parseXdUrl("xd://")).toEqual({ name: null });
		expect(parseXdUrl("xd://tools/grep")).toBeNull();
		expect(parseXdUrl("xd://skills/tacit-knowledge")).toBeNull();
	...
	it("XdProtocolHandler serves xd://skills/<name> from the session skill list", async () => {
		...
		).rejects.toThrow(/no skills loaded/);
	});
});
