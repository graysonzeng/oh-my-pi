...
describe("RuntimeAdapter", () => {
	...
	it.each([undefined, "custom-reviewer"] as const)(
		"preserves configured review models with agent override %s",
		async agent => {
			...
			const modelPattern = ["openai/gpt-6-astra", "gateway/grok-4.6"];
			await adapter.run(
				baseRequest(undefined, {
					role: "code_reviewer",
					agent,
					pipelineKind: "devflow",
					profile: { ...DEFAULT_MODEL_PROFILES.gpt_reviewer, modelPattern, strictIdentity: false },
					isolation: { requested: false, merge: "patch", apply: false },
				}),
			);
			expect(mapped?.agent).toBe(agent ?? "reviewer");
			expect(mapped?.model).toEqual(modelPattern);
		},
	);
	...
	it("maps workflow roles to registered bundled agent names", () => {
		expect(RuntimeAdapter.agentNameForRole("planner")).toBe("task");
