...
function fixtureLineageClass(provider: string): string {
...
export function scriptedRunner(script: {
	...
	return async request => {
		...
		let data: unknown;
		let label: string;
		if (request.workflowRole === "planner" || agent === "designer" || agent === "planner") {
			label = "plan";
			...
			(agent === "reviewer" && (script.gatePlanReview !== undefined || script.gateCodeReview !== undefined))
		) {
		...
		if (request.onResponse) {
			...
			const localModel = {
				...
				identity: { class: fixtureLineageClass(provider) },
				name: modelId,
