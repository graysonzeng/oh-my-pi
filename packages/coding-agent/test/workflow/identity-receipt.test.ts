...
describe("strict availability identity receipt", () => {
	...
	it.each(["header", "body"] as const)(
		"rejects a gateway model whose provider conflicts with %s metadata",
		async source => {
			const response = gatewayAttestationMetadata();
			if (source === "header") response.headers["x-omp-resolved-provider"] = "other-provider";
			else response.metadata = { provider: "other-provider" };
			const result = await probe(profile(), model(), [response]);
			expect(result.status).toBe("indeterminate");
			expect(result.identityProvenance).toBe("unknown");
			expect(result.exactIdentityMatch).not.toBe(true);
