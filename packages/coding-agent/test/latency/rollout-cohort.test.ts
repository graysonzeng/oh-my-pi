...
import {
	...
	getProcessScopedRolloutContext,
...
describe("LatencyRolloutCohortStore", () => {
	...
	it("does not mint a new bootNonce when using the process context", () => {
		const file = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "omp-nonce-")), "cohort.jsonl");
		const context = getProcessScopedRolloutContext();
		const store = new LatencyRolloutCohortStore(file);
		expect(getProcessScopedRolloutContext()).toBe(context);
		expect(store.bootNonce).toBe(context?.bootNonce ?? null);
	});
