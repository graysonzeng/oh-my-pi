...
describe("BashTool ledger completion integration", () => {
	...
	it("records timeout terminals from ACP create and poll paths", async () => {
		...
		const createResult = await new BashTool(createSession).execute("acp-create-timeout", {
			command: createCommand,
			timeout: 1,
		});
		expect(createResult.isError).toBe(true);
		expect(createResult.details?.timedOut).toBe(true);
		...
		const pollResult = await new BashTool(pollSession).execute("acp-poll-timeout", {
			command: pollCommand,
			timeout: 1,
		});
		expect(pollResult.isError).toBe(true);
		expect(pollResult.details?.timedOut).toBe(true);
