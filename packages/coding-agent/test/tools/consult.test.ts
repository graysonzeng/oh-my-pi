...
import { cfgConsultFirstEventTimeoutMs, cfgConsultTimeoutMs } from "@oh-my-pi/pi-coding-agent/config/workflow-settings";
...
describe("ConsultTool.execute", () => {
	...
	it("defaults consult wall-clock timeout to five minutes and first-event timeout to one minute", () => {
		expect(cfgConsultTimeoutMs.default).toBe(300_000);
		expect(cfgConsultFirstEventTimeoutMs.default).toBe(60_000);
	});
