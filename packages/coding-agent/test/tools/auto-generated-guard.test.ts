...
describe("assertEditableFile* without global Settings.init", () => {
	...
	it("assertEditableFileContent does not throw Settings-not-initialized", () => {
		const normal = "// Regular source\nexport const x = 1;";
		expect(() => assertEditableFileContent(normal, "ok.ts")).not.toThrow();
		// Default blockAutoGenerated is true — generated headers still blocked.
		const generated = "// Code generated by sqlc. DO NOT EDIT.\nexport const x = 1;";
		expect(() => assertEditableFileContent(generated, "gen.ts")).toThrow(ToolError);
	});
