...
describe("code_intel query tokens", () => {
	...
	it("ranks corpus anchors above unrelated files for English and Chinese queries", () => {
		...
		const wait = extractQueryTokens("where is isWaitingPollDetails for a wait snapshot of running jobs");
		expect(candidateScore("packages/tui/src/tools/wait.ts", "isWaitingPollDetails", wait)).toBeGreaterThan(
			candidateScore("packages/coding-agent/src/tools/grep.ts", "grep", wait),
		);
