...
describe("code_intel positive corpus", () => {
	...
	it("hits isWaitingPollDetails for wait snapshot queries", async () => {
		const english = await runQuery(REPO_ROOT, "where is isWaitingPollDetails for a wait snapshot of running jobs");
		expect(english.found).toBe(true);
		expectAnchor(english.evidence, "packages/tui/src/tools/wait.ts", "isWaitingPollDetails");
		expect(english.text).not.toContain("NOT_FOUND");

		const chinese = await runQuery(REPO_ROOT, "wait 的 isWaitingPollDetails 怎么判断 job 还在 running");
		expect(chinese.found).toBe(true);
		expectAnchor(chinese.evidence, "packages/tui/src/tools/wait.ts", "isWaitingPollDetails");
		expect(chinese.text).not.toContain("NOT_FOUND");
	});
