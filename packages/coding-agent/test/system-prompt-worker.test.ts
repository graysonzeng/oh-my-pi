...
describe("worker system prompt execution context", () => {
	...
	it("omits concise main-agent task delegation for workers", async () => {
		const worker = await render({ workerClass: true, model: "gateway/grok-4.6" });
		const main = await render({ workerClass: false, model: "gateway/grok-4.6" });

		expect(main).toContain("Handle bounded work directly");
		expect(main).not.toContain("§ Workflow");
		expect(worker).not.toContain("Handle bounded work directly");
		expect(worker).toContain("Use the available tools according to their contracts");
		expect(worker).toContain("`skill://tdd`");
		expect(worker).toContain("SAFETY_ALWAYS_APPLY_MARKER");
	});
