 ...
 * GPT-6.0 base (including `gpt-6-astra`), Grok 4.6, and DeepSeek V4 Flash
 * share concise guidance. Other models keep their existing template.
 */
export function getSystemPromptPolicy(modelId: string | undefined): SystemPromptPolicy {
	if (!modelId) return "default";
	const bareId = bareModelId(modelId);
	if (bareId === "grok-4.6" || bareId === "deepseek-v4-flash") return "concise";
