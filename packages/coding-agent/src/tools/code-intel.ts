...
import { cfgCodeIntelTimeoutSec } from "../config/workflow-settings";
...
function seedPathsFromSession(session: ToolSession, knownRelativePaths: readonly string[]): string[] {
	...
	for (const relative of knownRelativePaths) {
		const absolute = path.resolve(session.cwd, relative);
		if (!pathIsWithin(root, absolute)) continue;
		if (!store.headHash(absolute)) continue;
		const normalized = formatPathRelativeToCwd(absolute, session.cwd).replaceAll("\\", "/");
...
async function graphLayer(options: {
	...
	const seeds = seedPathsFromSession(options.session, files);
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			const timeoutSec = clampTimeout(
				"code_intel",
				cfgCodeIntelTimeoutSec.get(this.session.settings),
