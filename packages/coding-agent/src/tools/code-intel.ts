...
import { type CodeIntelIndexStatus, getCodeIntelIndex } from "./code-intel-index";
...
import { createCachedComponent, Ellipsis, formatCount, formatErrorMessage, replaceTabs } from "./render-utils";
...
async function grepLayer(options: {
	...
	for (const token of options.tokens.slice(0, 8)) {
		if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(token) && !/^[\u4e00-\u9fff]{2,}$/.test(token)) continue;
		for (const searchRoot of searchRoots) {
			try {
				...
				for (const match of result.matches) {
					...
					if (!decl) continue;
					const key = `${relative}:${match.lineNumber}:${decl.name}`;
...
async function tagsFromFiles(options: {
	...
	for (const file of rankedFiles.slice(0, 8)) {
		...
		try {
			...
			try {
				const extracted = await codeIntelExtractTags({ root: absolute, maxFiles: 1 });
...
async function graphLayer(options: {
	...
	const status = await index.ensureReady();
		...
		return untilAborted(signal, async () => {
			...
			try {
				...
				const grepResult = await grepLayer({
					...
					tokens,
					pathClue: params.path,
					extraRoots,
					signal: combined,
				});
				layers.grep = grepResult.used;
				candidates.push(...grepResult.candidates);
				...
				const graphResult = await graphLayer({
					...
					pathClue: params.path,
					relation,
					grepHits: grepResult.candidates,
					globFiles: grepResult.files,
					signal: combined,
				});
				layers.graph = graphResult.used;
				candidates.push(...graphResult.candidates);
				...
				candidates.push(...lspResult.candidates);
				...
				candidates.push(...semanticResult.candidates);
