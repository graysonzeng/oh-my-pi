...
import {
	cfgCodeIntelMaxEmbedFiles,
	cfgCodeIntelMaxIndexFiles,
	cfgCodeIntelTimeoutSec,
...
export class CodeIntelIndex {
	...
	async #buildNext(startedEpoch: number): Promise<void> {
		...
		const maxFiles = cfgCodeIntelMaxIndexFiles.get(this.#settings);
		const timeoutMs = Math.max(cfgCodeIntelTimeoutSec.get(this.#settings) * 1000, NATIVE_BUILD_MIN_TIMEOUT_MS);
				...
				if (!semantic) {
					...
					const outcome = await streamEmbedPassages({
						...
						limits: {
							...
							maxEmbedFiles: cfgCodeIntelMaxEmbedFiles.get(this.#settings),
						},
