...
/**
 * Prefer the global Settings singleton when initialized (CLI / Settings.init).
 * Isolated sessions (Settings.isolated without init — e.g. workflow-bench live)
 * never install the singleton; fall back to the schema default so edit/write
 * do not throw "Settings not initialized".
 */
function shouldBlockAutoGenerated(): boolean {
	try {
		return settings.get("edit.blockAutoGenerated") === true;
	} catch {
		return getDefault("edit.blockAutoGenerated") === true;
	}
}
