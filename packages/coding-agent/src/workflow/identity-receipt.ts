...
import { modelLineageClass } from "./model-lineage";
...
function normalizedAttestedIdentity(
	...
	const explicitProviders = new Set([
		...modelValues.map(value => parseModelString(value)?.provider).filter((value): value is string => Boolean(value)),
		...headerValues(PROVIDER_HEADERS),
		...metadataValues(["resolvedProvider", "provider", "providerId"]),
	]);
	if (explicitProviders.size > 1) return undefined;
	const gatewayModel = gatewayModels[0];
	const attestedModel = attestedModels.values().next().value;
	const attestedProvider = explicitProviders.values().next().value ?? localModel?.provider;
...
export function buildRuntimeIdentityReceipt(
	...
	const exactMatch =
		configured.provider && configured.model && attested.provider && attested.model
			...
			: null;
	const modelFamily = attested.model
		? (modelLineageClass(attested.model, attested.provider ?? undefined) ?? null)
		: null;
	return {
		schemaVersion: 1,
		configured,
