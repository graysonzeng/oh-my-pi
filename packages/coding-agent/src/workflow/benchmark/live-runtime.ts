...
function liveModelFamily(identity: LiveModelIdentityInput): string {
	return (
		modelLineageClass(identity.bareModel, identity.provider) ||
		modelLineageClass(identity.modelPattern, identity.provider) ||
...
export function buildLiveBenchmarkProfileOverrides(
	...
	for (const [id, base] of Object.entries(getDefaultConfig().profiles)) {
		...
		const liveIdentity: Partial<ModelProfile> = {
			vendor:
				modelLineageClass(target.bareModel, target.provider) ||
				modelLineageClass(target.modelPattern, target.provider) ||
				target.provider,
...
> {
	...
	const profileForRole = (role: WorkflowRole): string => {
		...
		const targetFamily = modelLineageClass(roleIdentityMap[role].model, roleIdentityMap[role].provider);
		const match = targetFamily
			? candidates.find(profile => {
					...
					return patterns.some(
						pattern => modelLineageClass(pattern, roleIdentityMap[role].provider) === targetFamily,
					);
				})
			: undefined;
