...
/** Workflow roles choose agent behavior; the configured profile owns model selection. */
export function resolvePipelineReviewAgent(
	role: WorkflowAgentRequest["role"],
	_opts?: PipelineReviewAgentOpts,
): string {
...
export class RuntimeAdapter implements RuntimePort {
	...
	async #runOnce<TArtifact>(
		...
		const mappedModel = request.profile.modelPattern;
