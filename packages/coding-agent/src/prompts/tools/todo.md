...
Before work, init for substantial multi-stage work or an explicit task-list request. Step count alone is not a trigger; bounded work proceeds directly even when it needs several tool calls. When using a list, MUST list EVERY user item separately (phased/numbered/bulleted/N); NEVER omit or remember leftovers.
