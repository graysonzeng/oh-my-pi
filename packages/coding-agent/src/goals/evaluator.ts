...
import { cfgGoalHostGateMaxOutputTokens } from "../config/workflow-settings";
...
export async function collectGoalGitSummary(cwd: string, signal?: AbortSignal): Promise<string> {
	try {
		...
		const [shortStatus, counts] = await Promise.all([repo.statusPorcelain({}, signal), repo.numstat({}, signal)]);
		const diffStat = counts
			.map(entry => `${entry.path} | +${entry.added ?? "-"} -${entry.removed ?? "-"}`)
			.join("\n");
...
export async function runGoalEvaluator(input: {
	...
	const maxTokens = cfgGoalHostGateMaxOutputTokens.get(input.session.settings) || DEFAULT_MAX_OUTPUT_TOKENS;
