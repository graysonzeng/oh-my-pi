...
import STRUCTURED_ASSISTANT_SUMMARY_PROMPT from "../prompts/compaction/structured-assistant-summary.md" with { type: "text" };
...
export class SessionMaintenance {
				...
				try {
					...
					if (retryFits) {
						this.#host.scheduleAgentContinue({
							source: "structured-compaction-retry",
							delayMs: 100,
	...
	maybeStartSpeculativeCompaction(contextTokens: number, contextWindow: number): void {
		...
		if (current) {
			...
			if (
				(growth <= refreshBudget ||
					(current.armed.kind === "compaction" &&
						isRecord(current.armed.result.preserveData?.anthropicCompaction))) &&
	...
	async checkCompaction(
		...
		const supersedeResult =
			this.#usesExperimentalContextManagement() || this.#structuredOwnsFirstOmission()
				? undefined
	...
	async runAutoCompaction(
		...
		for (let index = startIndex; index < methods.length; index++) {
			...
			if (candidate === "structured") {
				if (!this.#structuredRecoveryToolAvailable()) continue;
			} else if (
