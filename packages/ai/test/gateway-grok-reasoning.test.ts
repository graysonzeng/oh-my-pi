...
describe("gateway Grok cumulative reasoning", () => {
	...
	it("preserves additive Grok deltas on other providers even when prefixes overlap", async () => {
		const model = buildTestModel("grok-4.6", "xai");
		expect(await collectThinking(model, ["a", "ab"])).toBe("aab");
	});
});
