...
The body compresses hard. The trigger stays short.

- A tool's or skill's `description` field is **retrieval surface**, not documentation. Keep it natural, ≤160 characters: capability + narrow trigger + a `Not for …` exclusion when nearby tasks would otherwise fire it. Do not pad with keyword-redundant catchalls.
